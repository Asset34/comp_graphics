...
void UiLr1Controller::renderUi()
{
    ...
    m_rotationAngleChanged |= ImGui::SliderFloat("Angle", &m_rotationAngle, -360.0, 360.0, "%.0f deg");
    ImGui::SeparatorText("Line");
    m_lineBeginChanged |= ImGui::SliderFloat3("Line (a)", m_lineBegin, -200, 200);
    m_lineEndChanged |= ImGui::SliderFloat3("Line (b)", m_lineEnd, -200, 200);
    ImGui::End();
}
